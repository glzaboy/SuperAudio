...
SuperAudio（超级蓝牙音箱）是一款轻量级的 Windows 桌面应用程序，能够将您的电脑变身为蓝牙音频接收端。通过该应用，您可以将手机、平板等移动设备的音乐或音频流通过蓝牙传输到电脑的扬声器或耳机上进行播放，实现便捷的无线音频体验。

应用采用现代 Windows 11 设计语言，提供流畅的交互体验和丰富的自定义功能。
...
- **音频设备管理**：自动检测和管理系统音频播放设备，实时显示可用设备列表
- **音频播放连接**：支持启用和释放蓝牙音频播放连接
- **设备监控**：实时监控音频设备的插拔状态
- **热插拔支持**：设备插拔时自动更新设备列表，无需重启应用

### 界面特性

- **原生 Windows 11 风格**：采用 WinUI 3 构建的现代界面设计，遵循 Windows 11 设计规范
...
- **设置管理**：持久化用户偏好设置，支持 ApplicationData 和 JSON 两种存储方式
...
2. 安装以下工作负载：
   - ".NET 桌面开发"
   - "Windows 应用开发"（包含 WinUI 3 支持）
3. 克隆项目仓库
4. 使用 Visual Studio 打开解决方案文件（`SuperAudio.slnx`）
...
- **LoopbackRecorder**：循环录音服务，支持音频录制功能
